...
module.exports.Pattermer = class
{
    run() {
        ...
        this.activeVar = null; // currently selected variable element
        ...
        this.loadConfig();
        this.bindUIEvents();
        this.loadSet(this.cfg.activeSet);
    ...
    bindVariableSelectEvent(varElement) {
        varElement.addEventListener('mousedown', (e) => {
            ...
            if (this.activeVar) {
                this.activeVar.classList.remove('active');
            }

            e.currentTarget.classList.add('active');

            this.activeVar = varElement;
        });
    }
    ...
    bindAddVariableButton() {
        ...
        btnAddVar.addEventListener('click', (e) => {
            ...
            if (!this.cfg.sets[this.cfg.activeSet]['variables'][newVar]) {
                this.cfg.sets[this.cfg.activeSet]['variables'][newVar] = [];

                this.cfg.sets[this.cfg.activeSet]['variables'][newVar].push(newExpression);

                this.addVariableItem(newVar);
            } else {
                this.cfg.sets[this.cfg.activeSet]['variables'][newVar].push(newExpression);

                if (this.activeVar.textContent === newVar) {
    ...
    bindLoadPresetButton() {
        const btnLoadPreset = this.getElement('#btnLoadPreset');

        btnLoadPreset.addEventListener('click', (e) => {
            e.preventDefault();

            this.prompt('loadPreset', (formData) => this.loadSet(formData.txtSetName));
        });
    }
    ...
    bindSavePresetButton() {
        const btnSavePreset = this.getElement('#btnSavePreset');

        btnSavePreset.addEventListener('click', (e) => {
            e.preventDefault();

            this.prompt('savePreset', (formData) => {
                const setData = Object.assign({}, this.cfg.sets[this.cfg.activeSet]);

                delete(this.cfg.sets[this.cfg.activeSet]);

                this.cfg.activeSet = formData.txtSetName;
                this.cfg.sets[this.cfg.activeSet] = setData;
     ...
     */
    loadSet(setName) {
        if (!this.cfg.sets[setName]) {
            console.error('No preset found by name ' + setName);

            return;
        }

        for (const searchVar in this.cfg.sets[setName]['variables']) {
            this.addVariableItem(searchVar);
        }

        for (const varInQueue of this.cfg.sets[setName]['queue']) {
            this.addQueueItem(varInQueue);
        }
    ...
    displayVariableExpressions(varKey) {
        ...
        for (const expression of this.cfg.sets[this.cfg.activeSet]['variables'][varKey]) {
            this.addRegexpItem(expression);
        }
    }

    /**
     * Reconstructs filename preview by applying searches and replacements to the current list of files and
     * displays the list in the preview container
     *
    ...
    updateFilenamePreview(pattern) {
        ...
        for (const filename of this.files) {
            ...
            for (const queueItem of queueItems) {
                ...
                for (const varRegexp of this.cfg.sets[this.cfg.activeSet]['variables'][varPattern]) {
    ...
    prompt(templateName, successCallback) {
        ...
        modalWindow.children[0].appendChild(document.importNode(template.children[0].content, true));

        const btnSuccess = this.getElement('#btnModalSubmit'),
        ...
        modalWindow.classList.add('modalVisible');
    }

    // DOM UTIL METHODS

    /**
     * Deletes modal window's current contents and hides the modal
     */
    clearPrompt() {
        const modalWindow = this.getElement('#modalWindow'),
    ...
    readQueueOrder() {
        ...
        this.cfg.sets[this.cfg.activeSet]['queue'] = [];

        for (const queueItem of queueItems) {
            this.cfg.sets[this.cfg.activeSet]['queue'].push(queueItem.textContent);
        }
    }
