...
module.exports.Pattermer = class
{
    run() {
        ...
        // Dummy content
        this.searchVars = ['%author%', '%year%', '%location%'];
        this.expressions = {
            '%author%': ['(.*)_'],
            '%year%': ['\d{4}'],
            '%location%': ['\[\w+\]', '\(\w+\)', 'Paris']
    ...
    bindVariableClickEvents() {
        ...
        for (var i = 0; i < navVarItems.length; i++) {
            navVarItems[i].addEventListener('click', (e) =>
                this.displayVariableExpressions(e.currentTarget.textContent.trim())
            );
        }
    }
    ...
    loadSearchVariables() {
        ...
        for (let searchVar of this.searchVars) {
    ...
    displayVariableExpressions(varKey) {
        ...
        for (let regexp of this.expressions[varKey]) {
