...
module.exports.Pattermer = class
{
    ...
    bindVariablePaneDrags() {
        ...
        navVarContainer.addEventListener('drop', (e) => {
            if (this.dragOrigin === queueItemList) {
                ...
                if (this.findVarInQueue(varToRemove.textContent)) {
                    this.updateFilenamePreview();
                }
            }
    ...
    bindRemoveVariableButton() {
        this.getElement('#btnRemoveVar').addEventListener('click', (e) => {
            ...
            // If the removed variable was in the queue, remove it from the UI and update filename preview
            const queueItemToRemove = this.findVarInQueue(varToRemove.textContent);

            if (queueItemToRemove) {
                queueItemToRemove.parentNode.removeChild(queueItemToRemove);
