...
module.exports.Pattermer = class
{
    ...
    bindRegexpPaneDrags() {
        ...
        regexpList.addEventListener('dragstart', (e) => {
            e.dataTransfer.setData('regexpKey', this.getItemIndex(e.target));
    ...
    bindVariablePaneDrags() {
        ...
        navVarContainer.addEventListener('drop', (e) => {
            if (this.dragOrigin === queueItemList) {
                ...
                const varKey = this.getElement('#navigationVariables > span.active').textContent;

                this.currentPreset['variables'][varKey].splice(e.dataTransfer.getData('regexpKey'), 1);
