...
module.exports.Remmy = class
{
    ...
    bindVariablePaneDrags() {
        ...
        navVarContainer.addEventListener('drop', (e) => {
            if (this.dragOrigin === queueItemList) {
                ...
                // If the removed expression's host variable was in the queue, update filename preview
                if (this.isVarInQueue(this.selectedVar.textContent)) {
                    this.updateFilenamePreview();
                }
            }
    ...
    bindPatternChanges() {
        ...
        renamePattern.addEventListener('keyup', () => this.updateFilenamePreview());
        renamePattern.addEventListener('input', () => this.updateFilenamePreview());
    }
     ...
     * @var Node varNode
     */
    bindVariableSelect(varNode) {
        varNode.addEventListener('mousedown', (e) => {
            this.displayVariableExpressions(varNode.textContent);
    ...
    bindAddVariableButton() {
        ...
        this.getElement('#btnAddVar').addEventListener('click', (e) => {
            ...
            if (!this.currentPreset['variables'][newVar]) {
                ...
                // Update filename preview if the updated variable is in the queue
                const queueItems = this.getElements('#queueItems > li');

                for (const queueItem of queueItems) {
                    if (queueItem.textContent === newVar) {
                        this.updateFilenamePreview();
    ...
    bindRemoveVariableButton() {
        this.getElement('#btnRemoveVar').addEventListener('click', (e) => {
            ...
            // Remove variable from the queue and update filename preview
            let varIsInQueue = false;

            const queueItems = this.getElements('#queueItems > li');

            for (const queueItem of queueItems) {
                if (queueItem.textContent === varToRemove.textContent) {
    ...
    /**
     * Attaches event to perform renaming operation
     */
    bindApplyRenameButton() {
        this.getElement('#btnApplyRename').addEventListener('click', (e) => {
            e.preventDefault();

            const newFilenames = this.getTransformedFilenames(this.getElement('#txtRenamePattern').value);

            let success = true;

            for (const i in newFilenames) {
                const newPath = path.join(path.dirname(this.fileList[i]), newFilenames[i]);

                try {
                    fs.renameSync(this.fileList[i], newPath);
                } catch (err) {
                    success = false;

                    console.error('Error renaming a file: ' + err.message);
                }
            }
            ...
            if (success) {
                ...
                dialog.showErrorBox('Renaming error', 'Some of the files could not be renamed');
            }
     ...
     */
    bindHelpButton() {
        this.getElement('#btnHelp').addEventListener('click', (e) => {
            e.preventDefault();
    ...
    /**
     * Adds a regular expression item to their host container
     *
     * @param string itemText
     */
    addRegexpItem(itemText) {
        const regexpList = this.getElement('#regexpItems'),
            newId = 're-' + (regexpList.lastChild ? parseInt(regexpList.lastChild.id.split('-')[1]) + 1 : 0),
     ...
     * the current list of files and displays the new list in the preview container
     */
    updateFilenamePreview() {
        const pattern = this.getElement('#txtRenamePattern').value;

        this.displayFilenamePreview(pattern ? this.getTransformedFilenames(pattern) : []);
    }

    /**
     * Shows the list of new filenames in the filename preview table
     *
     * @param Array newFilenames
     */
    displayFilenamePreview(newFilenames) {
        const tBody = this.getElement('#previewTable > tbody'),
            btnApplyRename = this.getElement('#btnApplyRename');
        ...
        if (newFilenames.length === 0) {
            btnApplyRename.classList.add('hidden');
        } else if (!this.isFilenameSetValid(newFilenames)) {
            btnApplyRename.setAttribute('disabled', true);

            btnApplyRename.classList.remove('hidden', 'btn-positive');
            btnApplyRename.classList.add('btn-default');
        } else {
            btnApplyRename.removeAttribute('disabled');

            btnApplyRename.classList.remove('hidden', 'btn-default');
            btnApplyRename.classList.add('btn-positive');
        }
    }
    ...
    prompt(templateName, successCallback) {
        ...
        this.getElement('#modalWindow').children[0].appendChild(document.importNode(template.content, true));

        const btnSubmit = this.getElement('#btnModalSubmit'),
            btnCancel = this.getElement('#btnModalCancel');
        ...
        if (btnSubmit) {
            btnSubmit.addEventListener('click', (e) => {
                ...
                // Gather input values from the modal to pass onto the success function
    ...
    createDragsterZone(dropzone, nodeToMonitor) {
        ...
        // Reset dragster after a drop over the monitored node to prevent detection issues
    ...
    /**
     * Finds variable by var key from queue if it is there
     *
     * @param string varKey
     * @return bool
     */
    isVarInQueue(varKey) {
        ...
        for (const queueItem of queueItems) {
            if (queueItem.textContent === varKey) {
                return true;
            }
        }
    ...
    getTransformedFilenames(pattern) {
        ...
        for (const filePath of this.fileList) {
            ...
            newFilenames.push(sanitize(newFilename + extName));
        }
    ...
    /**
     * Determines whether the list of filenames is valid, i.e., it doesn't contain empty ones nor duplicates
     *
     * @param Array filenames
     * @return bool
     */
    isFilenameSetValid(filenames) {
        const set = new Set();
